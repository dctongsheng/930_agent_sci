import requests
import json
from example import plan_steps_1,plan_steps_2
from call_dify import get_filled_parametersv2
...
def output_node_info(dify_result):
    ...
    for i in dify_result:
        ...
        try:
            ...
            i_dict["previous_step"]=res_dependon["results"][0]["data"][0]["row"][0]["depend_on"]
    ...
    for i in raw_params:
        if i["step"] == 1:
            if i["plan_type"] == "wdl":
                ...
                data_choose=file_path
                i["raw_input_params"]=await get_filled_parametersv2(data_choose=data_choose,query_template=query_template,user=user,conversation_id=conversation_id,response_mode=response_mode)
...
def main(arg1:dict) -> dict:
    ...
    final_result_list=output_node_info(arg1["arg1"][0]["structured_output"]["required_steps"])

    final_result_list=asyncio.run(chuli_raw_planing(final_result_list,"/Users/zhaoyu/Desktop/test.h5ad"))


    return {"result":final_result_list}

if __name__ == "__main__":
    res = main(plan_steps_2)
    print(res)